...
from app.database import engine, get_db, DEV_DROP_RECREATE
...
async def lifespan(app: FastAPI):
    ...
    async with engine.begin() as conn:
        ...
        result_pers = await conn.execute(select(models.Kullanici).filter(models.Kullanici.email == "personel1@suski.gov.tr"))
        ...
        if not ist_result.fetchone():
            await conn.execute(
                models.Istasyon.__table__.insert().values([
                    {"ad": "Merkez Su Deposu", "konum": "Şanlıurfa Merkez", "tip": "Depo", "enlem": 37.1674, "boylam": 38.7955, "aktif_mi": True},
                    {"ad": "Karaköprü Kuyusu", "konum": "Karaköprü", "tip": "Kuyu", "enlem": 37.1901, "boylam": 38.7885, "aktif_mi": True},
                    {"ad": "Haliliye Şebeke", "konum": "Haliliye", "tip": "Şebeke", "enlem": 37.1583, "boylam": 38.8078, "aktif_mi": True}
                ])
            )
...
from app.auth import verify_password, create_access_token, ACCESS_TOKEN_EXPIRE_MINUTES, log_audit, require_role
...
async def create_istasyon(
    ...
    # Audit log
    await log_audit(db, kullanici_id=current_user.id, islem_tipi="ISTASYON_EKLENDI", detay=f"Yeni istasyon eklendi: {istasyon.ad}")
...
async def get_gis_istasyonlar(
    ...
    result = await db.execute(select(models.Istasyon).filter(models.Istasyon.aktif_mi == True))
...
@app.post("/su-olcumu", response_model=schemas.SuOlcumuResponse, tags=["Ölçümler"], description="Aşama 2 için /olcumler/ uç noktasının alias'ı")
async def create_olcum(
    ...
    # Audit log'a kaydet
    await log_audit(db, kullanici_id=current_user.id, islem_tipi="OLCUM_EKLENDI", detay=f"İstasyon {olcum.istasyon_id} için yeni ölçüm girildi.")
...
async def sim_tetikle(
    ...
    db_olcum = models.SuOlcumu(
        ...
        personel_notu=f"[SIM:{mod.upper()}] Otomatik simülasyon ölçümü",
        **veriler
    )
    db.add(db_olcum)
    await db.commit()
    ...
    await log_audit(db, kullanici_id=current_user.id,
...
async def get_su_olcumu_aksiyon_onerisi(
    ...
    return {
        "olcum_id": id,
        "aksiyon_onerisi": teknik_oneri,
        "llm_durumu": llm_durumu
    }