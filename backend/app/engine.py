...
async def hesapla_anomali_durumu(db: AsyncSession, olcum_verileri: dict):
    ...
    if ph is not None:
        if ph < 6.5:
            tespit_edilen_anomaliler.append({"kural": "Kritik Düşük pH", "mesaj": f"Asidik Su (pH: {ph}) - Boru korozyonu ve ağır metal çözünme riski!", "risk": "KRİTİK"})
        elif ph > 8.5:
            tespit_edilen_anomaliler.append({"kural": "Kritik Yüksek pH", "mesaj": f"Bazik Su (pH: {ph}) - Dezenfeksiyon verimsizliği!", "risk": "KRİTİK"})

    if klor is not None:
        if klor < 0.1:
            tespit_edilen_anomaliler.append({"kural": "Biyolojik Risk", "mesaj": f"Klor seviyesi çok yetersiz ({klor} mg/L)!", "risk": "KRİTİK"})
        elif klor < 0.2:
            tespit_edilen_anomaliler.append({"kural": "Düşük Klor", "mesaj": f"Dezenfeksiyon zayıf ({klor} mg/L).", "risk": "ORTA"})
        elif klor > 2.0:
            tespit_edilen_anomaliler.append({"kural": "Yüksek Klor", "mesaj": f"İdeal sınır aşıldı ({klor} mg/L).", "risk": "ORTA"})

    if bulan is not None:
        if bulan > 5.0:
            tespit_edilen_anomaliler.append({"kural": "Fiziksel Kirlilik", "mesaj": f"Kritik bulanıklık seviyesi ({bulan} NTU)!", "risk": "KRİTİK"})
        elif bulan > 1.0:
            tespit_edilen_anomaliler.append({"kural": "Yüksek Bulanıklık", "mesaj": f"Standart dışı bulanıklık ({bulan} NTU).", "risk": "ORTA"})

    if ilet is not None:
        if ilet > 800:
            tespit_edilen_anomaliler.append({"kural": "Mineral Anomalisi", "mesaj": f"Kritik seviye ({ilet} µS/cm)!", "risk": "KRİTİK"})
        elif ilet > 500:
            tespit_edilen_anomaliler.append({"kural": "Yüksek İletkenlik", "mesaj": f"Kabul edilebilir ancak yüksek ({ilet} µS/cm).", "risk": "ORTA"})
        elif ilet < 250:
            tespit_edilen_anomaliler.append({"kural": "Düşük İletkenlik", "mesaj": f"Saf su veya ölçüm hatası ihtimali ({ilet} µS/cm).", "risk": "ORTA"})
    ...
    for esik in esikler:
        ...
        if deger is not None:
            ...
            if esik.max_deger is not None and deger > esik.max_deger:
                ...
                tespit_edilen_anomaliler.append({"kural": "Üst Sınır İhlali", "mesaj": mesaj, "risk": "KRİTİK" if esik.parametre_adi == "ph" else "ORTA"})
        ...
        try:
            ...
            if eval(kural.kural_mantigi, {"__builtins__": {}}, clean_context):
                tespit_edilen_anomaliler.append({
                    "kural": kural.kural_adi,
                    "mesaj": kural.saha_uyarisi,
                    "risk": kural.risk_seviyesi
                })
