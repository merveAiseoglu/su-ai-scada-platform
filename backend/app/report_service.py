...
def generate_monthly_pdf_report(month_str: str, station_stats: list, judge_score_avg: float = None):
    ...
    sorted_stats = sorted(
        [s for s in station_stats if s["anomaly_rate"] > 0], key=lambda x: x["anomaly_rate"], reverse=True
    )[:5]
    if sorted_stats:
    ...
    if judge_score_avg is not None:
        elements.append(
            Paragraph(f"Average LLM Judge Score for {month_str}: {judge_score_avg:.1f} / 100", normal_style)
        )
    else:
        elements.append(
            Paragraph("No judge scores recorded for this period. (Pending judge-score tracking)", normal_style)
        )
